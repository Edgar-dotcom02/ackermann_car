...
def generate_launch_description():
    ...
    vehicle_controller_node = Node(package='ackermann_car',
                                   executable='vehicle_controller',
                                   parameters=[vehicle_params_path],
                                   output='screen')
    
    odom_node = Node(package='ackermann_car',
                        executable='odom_node',
                        parameters=[vehicle_params_path],
                        output='screen')
    
    # Wait for some time to run the path follower node
    path_follower = TimerAction(
        period=8.0,
        actions=[
            Node(package='ackermann_car',
